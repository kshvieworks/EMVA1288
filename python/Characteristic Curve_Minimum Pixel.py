...
class DarkCurrentAnalysis:
    ...
    def Calculate(self, ax, Frame, row, col):
        ...
        MinValues = np.array([])
        for k in range(Frame.__len__()):
            if MinValues.size == 0:
                MinValues = WH.ButtonClickedEvent.FindMinimumValues(Frame[k], col)
                MinValues = MinValues[:, np.newaxis]
            else:
                MinValues = np.append(MinValues, WH.ButtonClickedEvent.FindMinimumValues(Frame[k], col)[:, np.newaxis], axis=1)


        self.Label8_2_1.configure(text=f"{np.format_float_scientific(np.mean(MinValues), unique=False, precision=2)}")
        self.Label8_2_2.configure(text=f"{np.format_float_scientific(np.std(MinValues), unique=False, precision=2)}")
        self.Output = MinValues.copy()
    ...
    def __main__(self):
        ...
        Entry7Span = 2
        self.Label7_1_1 = tkinter.Label(self.InputinfoFrame, text='How many?')
        ...
        self.Button7 = tkinter.Button(self.InputinfoFrame, text='Min. Pixels')
