...
class DarkCurrentAnalysis:
    def __init__(self, window):
        self.window = window
        self.window.title("Photon Transfer Curve")
    ...
    def Show_ROI(self, Frame):
        ...
        self.variance_ij = (HF.DataProcessing.TemporalNoise(Frame, Differential=False))**2
        self.ROI_Data = Frame
    ...
    def Calculate(self, ax, Frame, variance_ij, row, col):

        Frame = HF.DataProcessing.TemporalAverage(Frame)

        self.Show_ROI(self.InputData.copy())
        self.ShowBlock(self.ROIWidget, Frame.copy(), row, col)
        ...
        Average = WH.ButtonClickedEvent.Average(ax, Frame.copy(), row, col)
        ...
        self.Output = np.append(Average[:, np.newaxis], stddev_y[:, np.newaxis], axis=1)

    def Apply_IQR(self, Frame, variance_ij, NIQR, NIteration, Widget, row, col):

        Frame = HF.DataProcessing.TemporalAverage(Frame)

        MaskedImage = WH.ButtonClickedEvent.IQR(Frame, NIQR, NIteration, False)
        WH.Plotting.ShowImage(HF.DataProcessing.TemporalAverage(MaskedImage), Widget)
        self.ShowBlock(Widget, MaskedImage.copy(), row, col)
        Average = WH.ButtonClickedEvent.Average(Widget, MaskedImage.copy(), row, col)
        ...
        self.Label10_2_1.configure(text=f"{np.format_float_scientific(np.mean(MaskedImage), unique=False, precision=2)}")
        self.Label10_2_2.configure(text=f"{np.format_float_scientific(np.sqrt(np.mean(variance_ij)), unique=False, precision=2)}")
    ...
    def SaveClipboardBTNEvent(self, data):
        df = pd.DataFrame(data, columns=['mean', 'stddev'])
    ...
    def __main__(self):
        ...
        self.Button8 = tkinter.Button(self.InputinfoFrame, text='Calculate',
                                      command=lambda: self.Calculate(self.ROIWidget, self.ROI_Data.copy(), self.variance_ij.copy(), int(self.Division_Row.get()), int(self.Division_Column.get())))
        ...
        Entry9Span = 2
        self.Button9 = tkinter.Button(self.InputinfoFrame, text='Apply IQR', command=lambda: self.Apply_IQR(self.ROI_Data.copy(),
                                                                                                            self.variance_ij.copy(),
                                                                                                            self.NIQR.get(),
                                                                                                            self.NIteration.get(),
                                                                                                            self.ROIWidget,
        ...
        self.Label10_1_1 = tkinter.Label(self.InputinfoFrame, text='Mean')
        self.Label10_1_1.grid(column=col, row=3)
        self.Label10_1_2 = tkinter.Label(self.InputinfoFrame, text='stddev')
        self.Label10_1_2.grid(column=col, row=4)
        self.Label10_2_1 = tkinter.Label(self.InputinfoFrame)
        self.Label10_2_1.grid(column=col+1, row=3)
